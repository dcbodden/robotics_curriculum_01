...
## Task 3.4: final curriculum review and validation summary

Completed on 2026-09-13. Reviewed the student README and teacher guide against the proposal, design, delta requirements, and recorded verification evidence.

- Student explanations use the two joystick controls, center-off motor behavior, observation tables, and 50 checks per second without requiring timer-register or asynchronous-programming knowledge. Added an explicit distinction between zero motor power at center and a servo that remains powered to hold its middle position.
- Both guides agree on VRx/A0, VRy/A1, motor D3, servo D9, orange signal/red external positive/brown shared ground, isolated positive supplies, protected MOSFET/diode wiring, and teacher-approved regulated 5 V actuator power. They include neutral checks, bounded travel, power-off uploads/changes, secured unloaded actuators, and troubleshooting for missing ground and unstable power.
- The four timing rhythms, buffered servo update, mechanical response, and foreground snapshots are distinguished consistently. Detailed Timer1 ownership, Servo library conflicts, ADC trade-offs, current-capacity selection, local decoupling, and branch routing remain teacher preparation topics. No exact servo angle, instantaneous stop, obstacle detection, or universal 1.5 A supply adequacy is promised.
- Checked all 23 local Markdown links and Markdown heading anchors across nine files: lesson index, lesson 05 README, lesson 06 README/teacher guide, and the change's proposal, design, tasks, delta spec, and verification record. All targets exist and anchors resolve. Existing worksheet PDF/HTML and teacher-guide links remain valid.
- Strict validation (`openspec validate add-joystick-motor-servo-lesson --strict`) and whitespace validation (`git diff --check`) passed. The task 3.1 Uno build and exhaustive mapping results remain applicable because no firmware changed during this final document review.

Completed evidence includes the Uno build (3150 bytes flash, 275 bytes static RAM), exhaustive mapping checks, source/generated-code timer and telemetry review, captured neutral-like serial commands, user-confirmed physical controls/neutral behavior, and reported successful operation with a 5 V, 1.5 A supply. Firmware, earlier lesson wiring, and the robot-attention worksheet were not changed during this review.

Remaining physical verification is explicitly unperformed: measured servo frame/pulse bounds, correlated command-to-frame latency, D3 waveform continuity, actual ISR duration, captured load-voltage stability during combined starts/movement, actual-part peak/stall current, and calculated supply margin. Hardware-specific supply documentation, branch ratings, and decoupling records were not supplied. These limitations remain visible in tasks 3.2/3.3 evidence and are not replaced by successful motion or document validation.
