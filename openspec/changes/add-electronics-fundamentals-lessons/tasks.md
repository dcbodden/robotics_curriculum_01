...
- [ ] 2.4 Add a DC voltage procedure that follows the resistance measurement, uses the black lead in COM and the red lead in the voltage jack, places the probes in parallel directly across B1's positive and negative terminals while the intentional S2-to-B1 gap remains open, and records B1's voltage.
- [ ] 2.5 Explain $V = I \times R$ and guide students to use B1's measured voltage and R1's measured resistance with consistent units to calculate and record predicted current in milliamps before any current-mode rewiring.
