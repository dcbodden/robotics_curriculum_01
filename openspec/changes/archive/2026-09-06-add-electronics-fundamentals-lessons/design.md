...
The student sequence is deliberate: measure R1 while the documented circuit keeps it electrically unpowered, measure the voltage provided by B1, calculate and record predicted current in milliamps with $I = V / R$, and only then remove the cells and reconfigure the meter in series to measure current. During resistance measurement, S2 remains released and the intentional gap between S2 and B1 positive remains open. These two open points prevent the battery from energizing R1 even when the AA cells remain installed. The worksheet will compare the measured result with the prediction using approximate agreement and consistent units rather than promise identical values. The teacher guide must verify the exact layout and both open points, actual B1 battery voltage, R1 resistance and power rating, expected circuit current, and a safe meter range before students begin.
...
Voltage measurement can be done directly on the B1 connections. Resistance remains an across-the-part measurement where no battery-powered path crosses R1: S2 is released, the intentional S2-to-B1-positive gap remains open, and a teacher confirms that no alternate path exists. The cells may remain installed for this exact resistance setup, but must be removed if the layout differs or either open point cannot be confirmed. A prominent warning will prohibit current-mode measurements across the battery and emphasize the importance of ensuring adequate load (resistance) in the circuit to avoid overcurrent situations.
...
- [A resistance reading includes an alternate or battery-powered path] → Use only the documented layout, keep S2 released and the S2-to-B1-positive gap open, and require a teacher to verify both open points and the absence of any alternate path; remove the cells if any condition is uncertain.
